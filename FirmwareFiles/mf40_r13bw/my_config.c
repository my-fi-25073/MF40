...
static void my_config_apply_defaults(my_config_t *config) {
    ...
    // keymap.c 기본 효과: LED0: Edge+Invert(12), LED1: Edge(4), LED2: Hold+Invert(9), LED3:none, LED4:caps
    config->led_flags[0] = (uint8_t)(BIT(2) | BIT(3));
    config->led_flags[1] = (uint8_t)(BIT(2));
    config->led_flags[2] = (uint8_t)(BIT(0) | BIT(3));
    config->led_flags[3] = 0u;
    config->led_flags[4] = 0u;

    // 인디케이터 기본: LED0:none(effect only), LED1:caps, LED2:num, LED3:none, LED4:caps
    config->indicator_source[0] = IND_NONE;
    config->indicator_source[1] = IND_CAPS;
    config->indicator_source[2] = IND_NUM;
    config->indicator_source[3] = IND_NONE;
    config->indicator_source[4] = IND_CAPS;
}
...
static void custom_config_get_value(uint8_t *data) {
    ...
    switch (*value_id) {
        case id_custom_led_flags_0:
            value_data[0] = my_config_get_led_flags(0);
            break;
        case id_custom_led_flags_1:
            value_data[0] = my_config_get_led_flags(1);
            break;
        case id_custom_led_flags_2:
            value_data[0] = my_config_get_led_flags(2);
            break;
        case id_custom_led_flags_3:
            value_data[0] = my_config_get_led_flags(3);
            break;
        case id_custom_led_flags_4:
            value_data[0] = my_config_get_led_flags(4);
            break;
        case id_custom_indicator_0:
            value_data[0] = my_config_get_indicator(0);
            break;
        case id_custom_indicator_1:
            value_data[0] = my_config_get_indicator(1);
            break;
        case id_custom_indicator_2:
            value_data[0] = my_config_get_indicator(2);
            break;
        case id_custom_indicator_3:
            value_data[0] = my_config_get_indicator(3);
            break;
        case id_custom_indicator_4:
            value_data[0] = my_config_get_indicator(4);
            break;
        default:
            break;
    }
...
static void custom_config_set_value(uint8_t *data) {
    ...
    switch (*value_id) {
        case id_custom_led_flags_0:
            my_config_set_led_flags(0, value_data[0]);
            break;
        case id_custom_led_flags_1:
            my_config_set_led_flags(1, value_data[0]);
            break;
        case id_custom_led_flags_2:
            my_config_set_led_flags(2, value_data[0]);
            break;
        case id_custom_led_flags_3:
            my_config_set_led_flags(3, value_data[0]);
            break;
        case id_custom_led_flags_4:
            my_config_set_led_flags(4, value_data[0]);
            break;
        case id_custom_indicator_0:
            my_config_set_indicator(0, value_data[0]);
            break;
        case id_custom_indicator_1:
            my_config_set_indicator(1, value_data[0]);
            break;
        case id_custom_indicator_2:
            my_config_set_indicator(2, value_data[0]);
            break;
        case id_custom_indicator_3:
            my_config_set_indicator(3, value_data[0]);
            break;
        case id_custom_indicator_4:
            my_config_set_indicator(4, value_data[0]);
            break;
        default:
            break;
    }
